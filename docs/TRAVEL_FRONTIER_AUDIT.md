...
- current explicit travel labels already understood by the compiler, including `ZL`, `Zone Line`, `Connection`, `Boundary`, `Portal`, `Teleport`, `Teleporter`, `Exit`, `Entrance`, and `To` forms;
- current candidates whose stored `zone_travel_edges` row is missing or stale;
- any additional explicit spellings registered outside the production parser as future review candidates; there are currently no such extra spellings after `Connection` / `Boundary` graduated into travel catalog v3;
- whether explicit destinations currently resolve to a unique canonical zone;
...
The audit intentionally does not promote remaining frontier evidence into routes. Parser expansion should be driven by real-corpus results and preserve EverQuestie's rule that ambiguous or weak evidence is not guessed. Bare zone-name labels remain audit-only.
