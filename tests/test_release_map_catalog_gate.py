...
class ReleaseMapCatalogPackagingContractTests(unittest.TestCase):
    def test_release_audits_finalized_snapshot_without_rebuilding_catalog(self):
        ...
        self.assertIn('map_catalog_sources = @("Goods", "Brewall")', script)
        ...
        hash_pos = script.index('$AuditedSnapshotHash =')
        windows_build_pos = script.index('& $WindowsBuilder @BuilderParams')
        archive_verify_pos = script.index('& $PythonCommand $ArchiveVerifier $Archive')
        self.assertLess(finalize_pos, reviewed_pos)
        self.assertLess(reviewed_pos, map_audit_pos)
        self.assertLess(map_audit_pos, hash_pos)
        self.assertLess(hash_pos, windows_build_pos)
        self.assertLess(windows_build_pos, archive_verify_pos)

    def test_full_build_audits_compiled_catalog_in_working_and_snapshot_databases(self):
        script = (REPO_ROOT / "tools" / "build_full_knowledge.ps1").read_text(
            encoding="utf-8"
        )

        self.assertIn('audit_map_catalog.py', script)
        self.assertIn('audit_map_catalog.py $WorkingDb', script)
        self.assertIn('audit_map_catalog.py $SnapshotDb', script)
        self.assertGreaterEqual(script.count('--require-source Goods'), 2)
        self.assertGreaterEqual(script.count('--require-source Brewall'), 2)
        self.assertGreaterEqual(script.count('--require-versioned-sources'), 2)
        self.assertIn('--output $MapCatalogAuditReport', script)
        self.assertIn('Map catalog       : verified portable + versioned', script)

        build_pos = script.index('python .\\tools\\build_knowledge_db.py')
        working_audit_pos = script.index('audit_map_catalog.py $WorkingDb')
        snapshot_audit_pos = script.index('audit_map_catalog.py $SnapshotDb')
        route_pos = script.index('python .\\tools\\audit_route_acceptance.py')
        self.assertLess(build_pos, working_audit_pos)
        self.assertLess(working_audit_pos, snapshot_audit_pos)
        self.assertLess(snapshot_audit_pos, route_pos)
