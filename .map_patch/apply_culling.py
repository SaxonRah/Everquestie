...
replace_once('import colorsys\nimport json\n', 'import colorsys\nimport heapq\nimport json\n', 'heapq import')
replace_once('from .db import Database\n', 'from .db import Database\nfrom .mapindex import SpatialMapIndex, bbox_intersects\n', 'mapindex import')
...
marker = '    def _on_elevation_changed(self) -> None:\n'
if marker not in s:
    raise SystemExit('render helper insertion marker missing')
helpers = '''    def _viewport_world_bounds(self, margin_px: float = 0.0) -> tuple[float, float, float, float]:
...
start = s.index('    def _rebuild_static(self) -> None:\n')
end = s.index('    def _apply_static_visibility(self) -> None:\n', start)
new_rebuild = '''    def _rebuild_static(self) -> None:
