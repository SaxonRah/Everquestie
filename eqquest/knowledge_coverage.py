...
class KnowledgeNormalizationCoverage:
    ...
    def provider(self, source_name: str, source_kind: str) -> ProviderNormalizationCoverage | None:
...
def _count_for_provider(
    ...
    *,
    ...
    row = db.conn.execute(
        ...
        FROM {table} value
        JOIN source_pages sp ON sp.id=value.source_page_id
...
def _breakdown(
    ...
    return tuple((str(row["label"] or "(unclassified)"), int(row["n"] or 0)) for row in rows)


def _normalized_page_count(db: DatabaseLike, source_name: str, source_kind: str) -> int:
    union_sql = "\nUNION\n".join(_DERIVATIVE_SOURCE_QUERIES)
    row = db.conn.execute(
        f"""
...
def provider_normalization_coverage(
    ...
    page_row = db.conn.execute(
        ...
        SELECT COUNT(*) AS source_pages,
               SUM(CASE WHEN trim(COALESCE(entity_type,''))<>'' THEN 1 ELSE 0 END) AS classified_pages
        FROM source_pages
    ...
    entity_links = _count_for_provider(
        ...
        source_name,
        source_kind,
        expression="COUNT(DISTINCT value.entity_id)",
    )
    primary_entity_links = _count_for_provider(
        ...
        source_kind,
        expression="COUNT(DISTINCT value.entity_id)",
        extra_where="value.role='primary'",
    )
...
def knowledge_normalization_coverage(db: DatabaseLike) -> KnowledgeNormalizationCoverage:
...
def normalization_coverage_text(
    ...
    *,
    ...
    lines += [
        ...
        "  • A normalized page has at least one canonical identity/graph/support derivative tied back to its source_page_id.",
